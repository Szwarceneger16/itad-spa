function* positionGenerator(radius, sectionsNumber) {
  ...
  const [pushBottomLimit, pushUpLimit] = [
    (70 * Math.PI) / 180,
    (110 * Math.PI) / 180,
  ];
  ...
  const crownsSections = Array(imageNumber)
    .fill("")
    .map((e, index) => {
      return {
...
function crownsGenerator(_refs, _CIRCLE_RADIUS, _CROWNS_NUMBER) {
  ...
  return [
    ...
    function (direction, crowns, data) {
      ...
      //     // debugger;
      //     // element.alt = data[val[index].name];
      //     // element.childNodes[0].onClick = data[val[index].callback];
      //     element.childNodes[0].style.transform =
      //       "rotate(" + val[index].rotate + "deg)";
      //   });
      return val;
    },
  ];
...
export { positionGenerator, disableScroll, enableScroll };
